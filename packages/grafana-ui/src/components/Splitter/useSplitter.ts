  ...
  const onUpdateSize = useCallback(
    (diff: number) => {
      ...
      if (usePixels) {
        ...
        secondPaneRef.current!.style.flexBasis = `${Math.max(newSize, 50)}px`; // Ensure minimum width of 50px
        ...
        const newFlex = Math.max(newSize / (containerSize.current! - handleSize), 0.05); // Ensure minimum flex size
